...
class UNetWithResnet50Encoder(pl.LightningModule):
    DEPTH = 6

    def __init__(self, n_classes=2, pretrained = True, num_input_channel = 3):
        super().__init__()
    ...
    def forward(self, x, with_output_feature_map=False):
        ...
        x = F.interpolate(x, size=self.closest_valid_dims, mode="nearest")
        ...
        x = torch.nn.self.out(x)
        x = F.interpolate(x, size=self.dims_before_reshape, mode="nearest")
